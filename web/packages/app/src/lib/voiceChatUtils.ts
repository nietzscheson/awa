...
export function isLikelyNonSpeechTranscript(raw: string): boolean {
  ...
  // ElevenLabs / Scribe-style scene captions (Spanish + English).
  if (
    /\bsonido\s+ambiente\b|\bruido\s+ambiental\b|\bruido\s+de\s+fondo\b|\bambient\s+(sound|noise)\b|\bbackground\s+noise\b/i.test(
      s,
    )
  ) {
    return true;
